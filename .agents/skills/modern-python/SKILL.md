...
metadata:
  ...
  adapted_for: TradingAgentsA
---

# Modern Python

> 来源：[trailofbits/skills](https://github.com/trailofbits/skills) 的 `modern-python`，原样引入。
>
> **本项目现状提示**：TradingAgentsA 当前用 Python **3.10** + **pip/setuptools**（`pyproject.toml`）+ 无 linter。
> 本 skill 推荐 uv/ruff/ty 和 Python 3.11+，是**升级方向参考**，不要在未与用户确认前擅自迁移工具链。
> 适合的场景：新建独立脚本（用 PEP 723）、用户主动要求引入 ruff/uv、配置 pytest 等。
