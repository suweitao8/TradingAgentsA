...
class ConfigService:
    ...
    # -----------------------------------------------------------------
    # 数据源/数据库测试的辅助方法（从 test_data_source_config / test_database_config 提取）
    # -----------------------------------------------------------------

    @staticmethod
    def _make_success(message: str, start_time: float, details: dict = None) -> Dict[str, Any]:
        """构造测试成功的统一返回结构（消除 30+ 处返回样板）"""
        return {
            "success": True,
            "message": message,
            "response_time": time.time() - start_time,
    ...
    @staticmethod
    def _make_failure(message: str, start_time: float, details: dict = None) -> Dict[str, Any]:
        """构造测试失败的统一返回结构（消除 30+ 处返回样板）"""
        return {
            "success": False,
            "message": message,
            "response_time": time.time() - start_time,
    ...
    @staticmethod
    def _classify_db_error(error_msg: str, db_type: str) -> str:
        """归类数据库连接错误，返回用户友好的提示（消除各 db 分支重复的错误判断链）"""
        msg_lower = error_msg.lower()
        if "authentication failed" in msg_lower or "auth failed" in msg_lower or "wrongpass" in msg_lower:
            return "认证失败，请检查用户名和密码"
        if "requires authentication" in msg_lower:
            return "需要认证，请配置用户名和密码"
        if "not authorized" in msg_lower or "access denied" in error_msg:
            return "权限不足，请检查用户权限配置"
        if "connection refused" in msg_lower or "can't connect" in msg_lower:
            return "连接被拒绝，请检查主机地址和端口"
        if "timed out" in msg_lower or "timeout" in msg_lower:
            return "连接超时，请检查网络和防火墙设置"
        if "no servers found" in error_msg:
            return "找不到服务器，请检查主机地址和端口"
        if "does not exist" in msg_lower or "unknown database" in msg_lower:
            return "数据库不存在"
        return f"连接失败: {error_msg}"

    async def _resolve_api_key(self, ds_config, env_var: str, start_time: float) -> tuple:
        """解析 API Key：配置值 → 数据库值 → 环境变量。返回 (api_key, used_db, used_env, error_result)。
        error_result 非 None 时表示解析失败，调用方应直接 return error_result。
        消除 tushare / alpha_vantage 间约 180 行逐字重复的凭证解析逻辑。"""
        import os
        api_key = ds_config.api_key or ""
        used_db = False
        used_env = False

        # 情况1：配置中的 API Key 包含 "..."（截断标记）
        if api_key and "..." in api_key:
            logger.info(f"🔍 [TEST] API Key contains '...' (truncated), checking database value")
            ...
            if db_config and db_config.api_key:
                ...
                if api_key == truncated_db_key:
                    ...
                    used_db = True
                    logger.info(f"✅ [TEST] Using complete API Key from database (length: {len(api_key)})")
                else:
                    return ("", False, False, self._make_failure(
                        "API Key 格式错误：检测到截断标记但与数据库中的值不匹配，请输入完整的 API Key",
                        start_time,
                        {"error": "truncated_key_mismatch", "received": api_key, "expected": truncated_db_key}
                    ))
            else:
                ...
                if env_key:
                    ...
                    used_env = True
                    logger.info(f"🔑 [TEST] Using {env_var} from environment (length: {len(api_key)})")
                else:
                    return ("", False, False, self._make_failure(
                        "API Key 无效：数据库和环境变量中均未配置有效的凭证", start_time
                    ))

        # 情况2：API Key 为空
        elif not api_key:
            logger.info(f"⚠️  [TEST] API Key is empty, trying database")
            system_config = await self.get_system_config()
            db_config = None
            if system_config:
                for ds in system_config.data_source_configs:
            ...
            if db_config and db_config.api_key and "..." not in db_config.api_key:
                ...
                if env_key:
                    ...
                    return ("", False, False, self._make_failure(
                        "API Key 无效：配置、数据库和环境变量中均未配置有效的凭证", start_time
                    ))
        else:
            logger.info(f"✅ [TEST] Using complete API Key from config (length: {len(api_key)})")

        return (api_key, used_db, used_env, None)

    # ===== 数据源 tester =====

    async def _test_tushare(self, ds_config, start_time: float) -> Dict[str, Any]:
        """测试 Tushare 数据源"""
        api_key, used_db, used_env, err = await self._resolve_api_key(ds_config, 'TUSHARE_TOKEN', start_time)
        if err:
            return err
        try:
            ...
            if df is not None and len(df) > 0:
                src = "数据库" if used_db else ("环境变量" if used_env else "配置")
                return self._make_success(
                    f"成功连接到 Tushare 数据源（使用{src}中的凭证）", start_time,
                    {"type": "tushare", "test_result": "获取交易日历成功",
                     "credential_source": src, "used_db_credentials": used_db, "used_env_credentials": used_env}
                )
            return self._make_failure("Tushare API 返回数据为空", start_time)
        except ImportError:
            return self._make_failure("Tushare 库未安装，请运行: pip install tushare", start_time)
        except Exception as e:
            return self._make_failure(f"Tushare API 调用失败: {str(e)}", start_time)

    async def _test_akshare(self, ds_config, start_time: float) -> Dict[str, Any]:
        """测试 AKShare 数据源"""
        try:
            ...
            if df is not None and len(df) > 0:
                return self._make_success(
                    "成功连接到 AKShare 数据源", start_time,
                    {"type": "akshare", "test_result": f"获取交易日历成功（{len(df)} 条记录）"}
                )
            return self._make_failure("AKShare API 返回数据为空", start_time)
        except ImportError:
            return self._make_failure("AKShare 库未安装，请运行: pip install akshare", start_time)
        except Exception as e:
            return self._make_failure(f"AKShare API 调用失败: {str(e)}", start_time)

    async def _test_baostock(self, ds_config, start_time: float) -> Dict[str, Any]:
        """测试 BaoStock 数据源"""
        try:
            ...
            if lg.error_code == '0':
                try:
                    ...
                    if rs.error_code == '0':
                        return self._make_success(
                            "成功连接到 BaoStock 数据源", start_time,
                            {"type": "baostock", "test_result": "登录成功，获取交易日历成功"}
                        )
                    return self._make_failure(f"BaoStock 数据获取失败: {rs.error_msg}", start_time)
                except Exception as e:
                    bs.logout()
                    return self._make_failure(f"BaoStock 数据获取异常: {str(e)}", start_time)
            return self._make_failure(f"BaoStock 登录失败: {lg.error_msg}", start_time)
        except ImportError:
            return self._make_failure("BaoStock 库未安装，请运行: pip install baostock", start_time)
        except Exception as e:
            return self._make_failure(f"BaoStock API 调用失败: {str(e)}", start_time)

    async def _test_yahoo_finance(self, ds_config, start_time: float) -> Dict[str, Any]:
        ...
        try:
            ...
            response = await _async_request("get", url, params={"interval": "1d", "range": "1d"}, timeout=10)
            if response.status_code == 200:
                data = response.json()
                if "chart" in data and "result" in data["chart"]:
                    return self._make_success(
                        "成功连接到 Yahoo Finance 数据源", start_time,
                        {"type": "yahoo_finance", "endpoint": ds_config.endpoint, "test_result": "获取 AAPL 数据成功"}
                    )
            return self._make_failure(f"Yahoo Finance API 返回错误: HTTP {response.status_code}", start_time)
        except Exception as e:
            return self._make_failure(f"Yahoo Finance API 调用失败: {str(e)}", start_time)

    async def _test_alpha_vantage(self, ds_config, start_time: float) -> Dict[str, Any]:
        """测试 Alpha Vantage 数据源"""
        api_key, used_db, used_env, err = await self._resolve_api_key(ds_config, 'ALPHA_VANTAGE_API_KEY', start_time)
        if err:
            return err
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if "Time Series (5min)" in data or "Meta Data" in data:
                    src = "数据库" if used_db else ("环境变量" if used_env else "配置")
                    return self._make_success(
                        f"成功连接到 Alpha Vantage 数据源（使用{src}中的凭证）", start_time,
                        {"type": "alpha_vantage", "endpoint": endpoint, "test_result": "API 密钥有效",
                         "credential_source": src, "used_db_credentials": used_db, "used_env_credentials": used_env}
                    )
                if "Error Message" in data:
                    return self._make_failure(f"Alpha Vantage API 错误: {data['Error Message']}", start_time)
                if "Note" in data:
                    return self._make_failure("API 调用频率超限，请稍后再试", start_time)
            return self._make_failure(f"Alpha Vantage API 返回错误: HTTP {response.status_code}", start_time)
        except Exception as e:
            return self._make_failure(f"Alpha Vantage API 调用失败: {str(e)}", start_time)

    async def _test_generic_http(self, ds_config, start_time: float) -> Dict[str, Any]:
        """测试其他数据源类型（通用 HTTP 探测，含 finnhub/polygon/iex/quandl 的环境变量映射）"""
        import os
        ds_type = ds_config.type.value if hasattr(ds_config.type, 'value') else str(ds_config.type)
        api_key = ds_config.api_key or ""
        used_env_credentials = False

        env_key_map = {"finnhub": "FINNHUB_API_KEY", "polygon": "POLYGON_API_KEY",
                       "iex": "IEX_API_KEY", "quandl": "QUANDL_API_KEY"}
        if ds_type in env_key_map and (not api_key or "..." in api_key):
            env_key = os.getenv(env_key_map[ds_type])
            if env_key:
                api_key = env_key.strip()
                used_env_credentials = True
                logger.info(f"🔑 使用环境变量中的 {ds_type.upper()} API Key ({env_key_map[ds_type]})")
        ...
        if ds_config.endpoint:
            try:
                ...
                if response.status_code < 500:
                    return self._make_success(
                        f"成功连接到数据源 {ds_config.name}", start_time,
                        {"type": ds_type, "endpoint": ds_config.endpoint,
                         "status_code": response.status_code, "used_env_credentials": used_env_credentials}
                    )
                return self._make_failure(f"数据源返回服务器错误: HTTP {response.status_code}", start_time)
            except Exception as e:
                return self._make_failure(f"连接失败: {str(e)}", start_time)
        return self._make_failure(f"不支持的数据源类型: {ds_type}，且未配置端点", start_time)

    # ===== 数据库 tester =====

    async def _test_mongodb(self, db_config, start_time: float) -> Dict[str, Any]:
        """测试 MongoDB 连接"""
        try:
            ...
            database, auth_source, used_env_config = db_config.database, None, False
            ...
            if not username or not password:
                ...
                if env_username and env_password:
                    username, password, auth_source, used_env_config = env_username, env_password, os.getenv('MONGODB_AUTH_SOURCE', 'admin'), True
            ...
            client.close()
            return self._make_success(
                "成功连接到 MongoDB 数据库", start_time,
                {"type": "mongodb", "host": host, "port": port, "database": database,
                 "auth_source": auth_source, "test_result": test_result, "used_env_config": used_env_config}
            )
        except ImportError:
            return self._make_failure("Motor 库未安装，请运行: pip install motor", start_time)
        except Exception as e:
            return self._make_failure(self._classify_db_error(str(e), "mongodb"), start_time)

    async def _test_redis(self, db_config, start_time: float) -> Dict[str, Any]:
        """测试 Redis 连接"""
        try:
            import redis.asyncio as aioredis
            import os
            host, port, password, database, used_env_config = db_config.host, db_config.port, db_config.password, db_config.database, False
            is_docker = os.path.exists('/.dockerenv') or os.getenv('DOCKER_CONTAINER') == 'true'
            if not password:
                env_password = os.getenv('REDIS_PASSWORD')
                if env_password:
                    password, used_env_config = env_password, True
            ...
            return self._make_success(
                "成功连接到 Redis 数据库", start_time,
                {"type": "redis", "host": host, "port": port, "database": database,
                 "redis_version": info.get("redis_version", "unknown"), "used_env_config": used_env_config}
            )
        except ImportError:
            return self._make_failure("Redis 库未安装，请运行: pip install redis", start_time)
        except Exception as e:
            return self._make_failure(self._classify_db_error(str(e), "redis"), start_time)

    async def _test_mysql(self, db_config, start_time: float) -> Dict[str, Any]:
        """测试 MySQL 连接"""
        try:
            import aiomysql
            conn = await aiomysql.connect(host=db_config.host, port=db_config.port, user=db_config.username,
                                          password=db_config.password, db=db_config.database, connect_timeout=5)
            async with conn.cursor() as cursor:
                await cursor.execute("SELECT VERSION()")
                version = await cursor.fetchone()
            conn.close()
            return self._make_success(
                "成功连接到 MySQL 数据库", start_time,
                {"type": "mysql", "host": db_config.host, "port": db_config.port,
                 "database": db_config.database, "version": version[0] if version else "unknown"}
            )
        except ImportError:
            return self._make_failure("aiomysql 库未安装，请运行: pip install aiomysql", start_time)
        except Exception as e:
            return self._make_failure(self._classify_db_error(str(e), "mysql"), start_time)

    async def _test_postgresql(self, db_config, start_time: float) -> Dict[str, Any]:
        """测试 PostgreSQL 连接"""
        try:
            import asyncpg
            conn = await asyncpg.connect(host=db_config.host, port=db_config.port, user=db_config.username,
                                         password=db_config.password, database=db_config.database, timeout=5)
            version = await conn.fetchval("SELECT version()")
            await conn.close()
            return self._make_success(
                "成功连接到 PostgreSQL 数据库", start_time,
                {"type": "postgresql", "host": db_config.host, "port": db_config.port,
                 "database": db_config.database, "version": version.split()[1] if version else "unknown"}
            )
        except ImportError:
            return self._make_failure("asyncpg 库未安装，请运行: pip install asyncpg", start_time)
        except Exception as e:
            return self._make_failure(self._classify_db_error(str(e), "postgresql"), start_time)

    async def _test_sqlite(self, db_config, start_time: float) -> Dict[str, Any]:
        """测试 SQLite 连接"""
        try:
            import aiosqlite
            ...
            async with aiosqlite.connect(db_path, timeout=5) as conn:
                async with conn.execute("SELECT sqlite_version()") as cursor:
                    version = await cursor.fetchone()
            return self._make_success(
                "成功连接到 SQLite 数据库", start_time,
                {"type": "sqlite", "database": db_path, "version": version[0] if version else "unknown"}
            )
        except ImportError:
            return self._make_failure("aiosqlite 库未安装，请运行: pip install aiosqlite", start_time)
        except Exception as e:
            return self._make_failure(f"连接失败: {str(e)}", start_time)

    async def test_data_source_config(self, ds_config: DataSourceConfig) -> Dict[str, Any]:
        """测试数据源配置 - 真实调用API进行验证（dispatch 到各 provider tester）"""
        start_time = time.time()
        ds_type = ds_config.type.value if hasattr(ds_config.type, 'value') else str(ds_config.type)
        logger.info(f"🧪 [TEST] Testing data source config: {ds_config.name} ({ds_type})")

        _testers = {
            "tushare": self._test_tushare,
            "akshare": self._test_akshare,
            "baostock": self._test_baostock,
            "yahoo_finance": self._test_yahoo_finance,
            "alpha_vantage": self._test_alpha_vantage,
        }
        tester = _testers.get(ds_type, self._test_generic_http)
        try:
            return await tester(ds_config, start_time)
        except Exception as e:
            logger.error(f"❌ 测试数据源配置失败: {e}")
            return self._make_failure(f"连接失败: {str(e)}", start_time)

    async def test_database_config(self, db_config: DatabaseConfig) -> Dict[str, Any]:
        """测试数据库配置 - 真实连接测试（dispatch 到各 db tester）"""
        start_time = time.time()
        ...
        if not tester:
            return self._make_failure(f"不支持的数据库类型: {db_type}", start_time)
        try:
            return await tester(db_config, start_time)
        except Exception as e:
