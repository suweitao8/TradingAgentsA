...
TradingAgentsA 提供了统一的配置系统，所有配置通过 `.env` 文件管理。本指南详细介绍了所有可用的配置选项和最佳实践，包括v0.1.7新增的Docker部署和报告导出配置。
...
通过合理的配置，您可以根据不同的使用场景优化 TradingAgentsA 的性能和成本。
